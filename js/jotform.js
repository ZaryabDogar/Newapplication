            ...
            highLightableList.forEach(highLightable => {
                highLightable.addEventListener('focus', () => {
                    ...
                    if (isNotCollapsingSection || (!isFormSectionRelated && isCollapsedLine)) {
                        const collapseBar = JotForm.getCollapseBar(isCollapsedLine);
                        collapseBar.dispatchEvent(new Evenet('click'));
                    }
    ...
    validator: function () {
        ...
        $A(JotForm.forms).each(function (form) { // for each JotForm form on the page
            ...
            var handleFormSubmit = function (e) {
                ...
                JotForm.sendFormOpenId(form, 'clientSubmitFinish_V4');
  ...
  /**
    ...
    sendFormOpenId: function (form, type) {
        try {
            ...
            var openFormIdInput = form.querySelector('[name="formOpenId_V4"]');
            if (!openFormIdInput) return;
            fetch(JotForm.getAPIEndpoint() + '/form/' + form.id + '/event/' + openFormIdInput.value + '/' + type, { method: 'POST', keepalive: true });
        } catch (error) {
